...
public class GameObjectGenerator {
	...
	public static void generateGameObjects(Game game, Level level) {
		...
		for(int x = game.getVisibility() /2; x < game.getRoadLength(); x ++) {
			...
			if (level.hasAdvancedObjects()) {
				...
				game.tryToAddObject(new Tank(game, x, game.getRandomLane()), level.advancedObjectsFrequency());
	...
	public static void forceAdvanceObject(Game game, int id, int x) throws UnvalidCheatNumberException {
		GameObject o = null;
		switch (id) {
		case 1:
		...
		case 5:
			...
			break;
		default:
			throw new UnvalidCheatNumberException("[ERROR]: Cheat Command : Unvalid argument for cheat command, number expected: [1..5]");
			}
